...
toggleterm.setup {
	...
	on_open = function(term)
		vim.api.nvim_buf_set_keymap(term.bufnr, 'n', '<leader>tq', ':q!<CR>',
			{ noremap = true, desc = 'Kill terminal instance' })

		-- Clear keymaps that allow us to open other files in our terminal buffer
		vim.api.nvim_buf_set_keymap(term.bufnr, 'n', '<leader>fB', '',
			{ noremap = true, desc = '' })
		vim.api.nvim_buf_set_keymap(term.bufnr, 'n', '<leader>fb', '',
			{ noremap = true, desc = '' })
		vim.api.nvim_buf_set_keymap(term.bufnr, 'n', '<leader>ff', '',
			{ noremap = true, desc = '' })
	end,
}
